...
# the dataset to be used for annotation using biomart:for human "hsapiens_gene_ensembl"
dataset_mart<-"hsapiens_gene_ensembl" 
...
ENSG_mapping <- read.delim("annotationData/chipTypes/hta20_Hs_ENSG_19/hta20_Hs_ENSG_mapping.txt")
colnames(ENSG_mapping)[1] <- 'gene'
ENSG_mapping$gene <- gsub('_at','',ENSG_mapping$gene)
...
ENSE_Mapping <- read.delim("annotationData/chipTypes/hta20_Hs_ENSE_19/hta20_Hs_ENSE_mapping.txt")
colnames(ENSE_Mapping)[1] <- 'exon'
ENSE_Mapping$exon <- gsub('_at','',ENSE_Mapping$exon)

#Load annotation file containing list of exons and corresponding genes 
ENSE_desc <- read.delim("annotationData/chipTypes/hta20_Hs_ENSE_19/hta20_Hs_ENSE_desc.txt")

gene_exon <- data.frame(str_extract(ENSE_desc$Description, '\\ENSG...........'),ENSE_desc$Probe.Set.Name)
colnames(gene_exon)<- c('gene','exon')
gene_exon$exon <- gsub('_at','',gene_exon$exon)

# exons per gene
ge_ex_DT <- data.table(gene_exon, key='gene')
...
#Name of the cdf file  - gene level  - for example for brain array gene level: hta20_Hs_ENSG_19
chipType_gene <- 'hta20_Hs_ENSG_19'
#Name of the cdf file  - exon level - for example for brain array gene level: hta20_Hs_ENSE_19
chipType_exon <- 'hta20_Hs_ENSE_19'
...
convertCdf("annotationData/chipTypes/hta20_Hs_ENSG_19/hta20_Hs_ENSG_19.cdf","annotationData/chipTypes/hta20_Hs_ENSG_19/hta20_Hs_ENSG_19,binary.cdf",verbose=TRUE)  
convertCdf("annotationData/chipTypes/hta20_Hs_ENSE_19/hta20_Hs_ENSE_19.cdf","annotationData/chipTypes/hta20_Hs_ENSE_19/hta20_Hs_ENSE_19,binary.cdf",verbose=TRUE)  
...
trFit$gene <- gsub('_at','',trFit$gene)
...
plmTr_exon <- ExonRmaPlm(csN_exon, mergeGroups=TRUE)
...
trFit_exon$exon <- gsub('_at','',trFit_exon$exon)
...
grA_ex_sd$gene <- with(gene_exon, gene_exon[,1][match(gene_exon[,2], exon)]) # add genes
setkey(grA_ex_sd, key='gene')

grB_ex_sd <- data.table(exon=as.character(trFit_exon_sd[,1]), grB_sd=trFit_exon_sd$sd_exon_grB)
grB_ex_sd$gene <- with(gene_exon, gene_exon[,1][match(gene_exon[,2], exon)]) # add genes
...
sd_threshold <- 5 #threshold for SD: step 1 filter to remove invariant genes, user can change this based on distribution plot of SD

nSamples <- 10000 #establishing FDR for MUF scores - number of permutations per probe_number

fdr_threshold_value <- 0.05 #fdr threshold value: step 2 filter for MUF scores
...
outs <- which(gsub('_at','',names(new_rsu))%in%all_out_genes[,1])
...
#Calculate number of probes per Gene and remove genes with less then 3 probe from new_rsu
nProbe <- sapply(new_rsu, FUN = function(u) nrow(u))
minProbes <- 3
feature_keep <- which(nProbe>=minProbes)

new_rsu_keep<-new_rsu[feature_keep]
...
# now applying to gene filtered new_rsu_keep
rsu1 <- lapply(new_rsu_keep, FUN = function(u, cls) {
  r <-log2(u)
  calcMeans_median(r, cls)
}, cls = cls)

# Calculate muf scores
nProbe <- sapply(new_rsu_keep, FUN = function(u) nrow(u))
...
row.names(fdr_df) <- gsub('_at','',row.names(fdr_df))
...
ge <- merge(gene_exon, trFit, by='gene')
...
# please check if strand annotation is '+' and '-' or '1' and '-1' in the exon_map object:change exon_map$Chr.Strand == '-1' accordinngly
exon_map_neg<- subset(exon_map, exon_map$Chr.Strand == '-1')
exon_map_pos<- subset(exon_map, exon_map$Chr.Strand == '1')
...
# please check if strand annotation is '+' and '-' or '1' and '-1' in the exon_map object:change exon_map$Chr.Strand == '-1' accordinngly
exon_df_neg<- subset(exon_df, exon_df$Chr.Strand == '-1')
exon_df_pos<- subset(exon_df, exon_df$Chr.Strand == '1')
...
gene <- 'ENSG00000005471' # pick a gene
